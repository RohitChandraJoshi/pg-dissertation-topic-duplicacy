...
function App() {
  ...
  useEffect(() => {
    axios.get('http://localhost:8080/get-random-topic')
      .then((response) => {
  ...
  const handleCompare = () => {
    axios.get('http://localhost:8080/get-topics')
      .then((response) => {
        ...
        if (similarityPercentage >= 70) {
          ...
          axios.post('http://localhost:8080/submit-topic', { topic: enteredTopic, guide: selectedGuide })
            .then((response) => {
